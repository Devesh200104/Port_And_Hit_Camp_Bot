...
def display_pannel():
    ...
    def Settings():
        ...
        def swipe_finder():
            val=swipe_var.get()
            if val==1:
                my_file=open("S_Depe_2/swipe_method.txt", mode="w")
        ...
        def port_type_finder():
            val=port_var.get()
            if val==1:
                my_file=open("Depe_2/Settings/port_type.txt", mode="w")
                my_file.write(str('random'))
                my_file.close() 
            else:
                my_file=open("Depe_2/Settings/port_type.txt", mode="w")
                my_file.write(str('precise'))
                my_file.close() 
        ...
        def spawn_finder():
            ...
            if val==1:
                my_file=open("Depe_2/Settings/hit_at_spawn.txt", mode="w")
        ...
        def game_finder():
            ...
            if val==1:
                my_file=open("Depe_2/Settings/start_game.txt", mode="w")
                my_file.write(str('yes'))
                my_file.close() 
        ...
        def spawn_time():
            ...
            if val1!='' and val2!='':
                ...
                my_file=open("Depe_2/Settings/spawn_time.txt", mode="w")
                my_file.write(str(val))
                my_file.close() 


        def map_scanner():
            val=map_scanner_var.get()
            if val==1:
                my_file=open("Depe_2/Settings/map_scanner.txt", mode="w")
                my_file.write(str('star'))
                my_file.close() 
            else:
                my_file=open("Depe_2/Settings/map_scanner.txt", mode="w")
                my_file.write(str('zigzag'))
                my_file.close() 
        ...
        ## Swipe
        label_swipe=Label(all_frame,text="Swipe Speed : ",fg="#990000").grid(row=0,column=0)
        
        options= [["Slow ",1],["Fast ",2]]
        swipe_var=IntVar() 
        ...
        for text, val in options :
            R_button= Radiobutton(all_frame,text=text,variable=swipe_var,value=val).grid(row=0,column=c)
            c+=1
        ...
        label_port=Label(all_frame,text="Port type : ",fg="#990000").grid(row=2,column=0)
        
        options= [["Random ",1],["Precise ",2]]
        port_var=IntVar() 
        
        f=open("Depe_2/Settings/port_type.txt","r")
        port_val=f.read()
        f.close()
        ...
        c=1
        for text, val in options :
            R_button= Radiobutton(all_frame,text=text,variable=port_var,value=val).grid(row=2,column=c)
            c+=1
        ...
        f=open("Depe_2/Settings/start_game.txt","r")
       ...
       ## Map Scanner Method
        ...
        label_scanner=Label(all_frame,text="Map Scan Type : ",fg="#990000").grid(row=6,column=0,pady=10)
        options= [["star ",1],["zigzag ",2]]
        map_scanner_var=IntVar() 
        
        f=open("Depe_2/Settings/map_scanner.txt","r")
        map_scanner_val=f.read()
        f.close()
        
        if map_scanner_val=='star':
            map_scanner_var.set(1)
        else:
            map_scanner_var.set(2)
            
        c=1
        for text, val in options :
            R_button= Radiobutton(all_frame,text=text,variable=map_scanner_var,value=val).grid(row=6,column=c)
            c+=1 
        ...
        f=open("Depe_2/Settings/spawn_time.txt","r")
        time_=f.read()
        time_=time_.split(":")
        f.close()
        ...
        notice_label=Label(all_frame,text="You Just Need To Write One Spawn Time. It Will calculate other spawn times and hit accordingly",fg="#990000").grid(row=8,column=0,columnspan=5)
    ...
    Entry_Frame = LabelFrame(root,text="Select Duration To Run",padx=20,pady=20)
...
def windows_1920(cmd_number,settings,server_arr,discord_key_arr):
    ...
    def porter(acc_porting,port_area):
    ...
    def beside_port(cords_camps):
        ...
        for i in range(tot_no):
            ...
            if porter(acc_porting,area)==False:
                continue
            else:
                f=True
                break
